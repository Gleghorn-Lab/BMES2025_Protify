...
kingdom_df = pd.concat([filtered, tax_id_df], axis=1)

# Create kingdom dataset (kingdom name and sequence)
kingdom_df = kingdom_df[['Entry','kingdom', 'Sequence']].copy()
kingdom_df = kingdom_df.dropna()
print(f"Number of unique kingdoms: {kingdom_df['kingdom'].nunique()}") #prints 20
print(f"Total number of rows: {len(kingdom_df)}") #prints 557,052
print(kingdom_df.head())
kingdom_df.to_csv('kingdom.tsv', sep='\t', index=False)
print(f"Filtered entries saved to {"kingdom.tsv"}")

#convert the tsv file to a fasta format
with open("kingdom.tsv", newline='') as tsv, open("kingdom.fasta", 'w') as fasta:
    reader = csv.DictReader(tsv, delimiter='\t')
    for row in reader:
        # build a FASTA header:
        header = f">{row['Entry']}|{row['kingdom']}"
        seq    = row['Sequence']
...
fasta_file = 'kingdom.fasta'
cd_hit(fasta_file, similarity_threshold=0.5, n=2, memory_percentage=0.5)

#RESULT - output_kingdom_0.5.fasta file showing the representative seqeunces and output_kingdom_0.5.clstr showing the cluster results from CD-HIT
#Read the FASTA into a list of dicts
with open("output_kingdom_0.5", "r") as f:
    lines = f.read().splitlines()

records = []
entry, kingdom, sequence = "", "", ""
...
df = df.drop_duplicates(subset=['kingdom','Sequence'], keep='first') #no duplicates found
print(f"After dropping duplicates: {len(df)}")
##RESULT - no duplicated found or removed

print(f"Number of unique kingdom: {df['kingdom'].nunique()}") #prints 20
print(f"Total number of rows: {len(df)}") #prints 262,596

# Remove fewer than 100 samples
kingdom_counts = df['kingdom'].value_counts()
df = df[df['kingdom'].isin(kingdom_counts[kingdom_counts >= 100].index)]
print(f"Number of unique kingdom after removing rare kingdoms: {df['kingdom'].nunique()}") #prints 16
print(f"Total number of rows after removing rare kingdom: {len(df)}") #prints 262,2442

# Create a new 'label' column based on species
df["labels"] = pd.factorize(df["kingdom"])[0]
print(df.head())

output_filename = 'kingdom_labeled.csv'

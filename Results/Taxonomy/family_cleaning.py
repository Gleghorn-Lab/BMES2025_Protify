...
family_df = pd.concat([filtered, tax_id_df], axis=1)

# Create family dataset (family name and sequence)
family_df = family_df[['Entry','family', 'Sequence']].copy()
family_df = family_df.dropna()
print(f"Number of unique family: {family_df['family'].nunique()}") #prints 1,918
print(f"Total number of rows: {len(family_df)}") #prints 564,101
print(family_df.head())
family_df.to_csv('family.tsv', sep='\t', index=False)
print(f"Filtered entries saved to {"family.tsv"}")

#convert the tsv file to a fasta format
with open("family.tsv", newline='') as tsv, open("family.fasta", 'w') as fasta:
    reader = csv.DictReader(tsv, delimiter='\t')
    for row in reader:
        # build a FASTA header:
        header = f">{row['Entry']}|{row['family']}"
        seq    = row['Sequence']
...
# Remove species/classes with fewer than 2 samples

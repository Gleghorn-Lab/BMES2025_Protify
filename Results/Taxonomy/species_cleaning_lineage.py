...
species_df = pd.concat([filtered, tax_id_df], axis=1)

# Create species dataset (species name and sequence)
species_df = species_df[['Entry','species', 'Sequence']].copy()
species_df = species_df.dropna()
print(f"Number of unique species: {species_df['species'].nunique()}") #prints 2,256
print(f"Total number of rows: {len(species_df)}") #prints 378,178
print(species_df.head())
species_df.to_csv('species.tsv', sep='\t', index=False)
print(f"Filtered entries saved to {"species.tsv"}")

#convert the tsv file to a fasta format
with open("species.tsv", newline='') as tsv, open("species.fasta", 'w') as fasta:
    reader = csv.DictReader(tsv, delimiter='\t')
    for row in reader:
        # build a FASTA header:
        header = f">{row['Entry']}|{row['species']}"
        seq    = row['Sequence']
...
#RESULT - output_species_0.8.fasta file showing the representative seqeunces and output_species_0.8.clstr showing the cluster results from CD-HIT
#Read the FASTA into a list of dicts
with open("output_species_0.5", "r") as f:
    lines = f.read().splitlines()

records = []
entry, species, sequence = "", "", ""
...
df = df.drop_duplicates(subset=['species','Sequence'], keep='first') #no duplicates found
print(f"After dropping duplicates: {len(df)}")

print(f"Number of unique species: {df['species'].nunique()}") #prints 2148
print(f"Total number of rows: {len(df)}") #prints 175,505

# Remove species/classes with fewer than 100 samples
species_counts = df['species'].value_counts()
df = df[df['species'].isin(species_counts[species_counts >= 100].index)]
print(f"Number of unique species after removing rare classes: {df['species'].nunique()}") #prints 506
print(f"Total number of rows after removing rare classes: {len(df)}") #prints 157,523

# Create a new 'label' column based on species
df["labels"] = pd.factorize(df["species"])[0]
print(df.head())

output_filename = 'species_labeled.csv'

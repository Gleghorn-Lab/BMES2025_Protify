...
genus_df = pd.concat([filtered, tax_id_df], axis=1)

# Create genus dataset (genus name and sequence)
genus_df = genus_df[['Entry','genus', 'Sequence']].copy()
genus_df = genus_df.dropna()
print(f"Number of unique genuses: {genus_df['genus'].nunique()}") #prints 5,354
print(f"Total number of rows: {len(genus_df)}") #prints 566,880
print(genus_df.head())
genus_df.to_csv('genus.tsv', sep='\t', index=False)
print(f"Filtered entries saved to {"genus.tsv"}")

#convert the tsv file to a fasta format
with open("genus.tsv", newline='') as tsv, open("genus.fasta", 'w') as fasta:
    reader = csv.DictReader(tsv, delimiter='\t')
    for row in reader:
        # build a FASTA header:
        header = f">{row['Entry']}|{row['genus']}"
        seq    = row['Sequence']

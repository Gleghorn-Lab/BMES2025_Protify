...
domain_df = pd.concat([filtered, tax_id_df], axis=1)

# Create domain dataset (domain name and sequence)
domain_df = domain_df[['Entry','domain', 'Sequence']].copy()
domain_df = domain_df.dropna()
print(f"Number of unique domains: {domain_df['domain'].nunique()}") #prints 3
print(f"Total number of rows: {len(domain_df)}") #prints 550,361
print(domain_df.head())
domain_df.to_csv('domain.tsv', sep='\t', index=False)
print(f"Filtered entries saved to {"domain.tsv"}")

#convert the tsv file to a fasta format
with open("domain.tsv", newline='') as tsv, open("domain.fasta", 'w') as fasta:
    reader = csv.DictReader(tsv, delimiter='\t')
    for row in reader:
        # build a FASTA header:
        header = f">{row['Entry']}|{row['domain']}"
        seq    = row['Sequence']
...
fasta_file = 'domain.fasta'
cd_hit(fasta_file, similarity_threshold=0.8, n=5, memory_percentage=0.5)
"""
#RESULT - output_domain_0.8.fasta file showing the representative seqeunces and output_domain_0.8.clstr showing the cluster results from CD-HIT
#Read the FASTA into a list of dicts
with open("output_domain_0.8", "r") as f:
    lines = f.read().splitlines()

records = []
entry, domain, sequence = "", "", ""
...
df["labels"] = pd.factorize(df["domain"])[0]
print(df.head())

print(f"Before dropping duplicates: {len(df)}")
df = df.drop_duplicates(subset=['domain','Sequence'], keep='first') #no duplicates found
print(f"After dropping duplicates: {len(df)}")
##RESULT - no duplicated found or removed
output_filename = 'domain_labeled.csv'
df.to_csv(output_filename, index=False)
print(df.head())

print(f"Number of unique domains: {df['domain'].nunique()}") #prints 116
print(f"Total number of rows: {len(df)}") #prints 269,838

# Remove fewer than 100 samples
domain_counts = df['domain'].value_counts()
df = df[df['domain'].isin(domain_counts[domain_counts >= 100].index)]
print(f"Number of unique domains after removing rare domains: {df['domain'].nunique()}") #prints 3
print(f"Total number of rows after removing rare domains: {len(df)}") #prints 261,620
